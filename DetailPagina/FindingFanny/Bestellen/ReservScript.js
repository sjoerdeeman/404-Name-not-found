    ...
    function GoThroughRij() {
        ...
        for (ab = 0 ; ab < RijLength ; ab++) {
            ...
            for (a = 0 ; a < rij[ab][0]; a++) {
                id++;
                var NoChairOutput = '<div class="NoChair" style="float:left; background-color:transparent; width:20px;" id="' + id + '"><p class="invi"> </p></div>';
                OutputDiv.innerHTML += NoChairOutput;
            }
            //stoelen klasse 1 links vh midden
            for (b = 0 ; b < rij[ab][1]; b++) {
                id++;
                var ChairCat1Output = '<div class="Chair"  onmousedown="javascript:ShowPrice(1,' + id + ')"id="' + id + '"><p style="color:#839C36;"> </p></div>';
                OutputDiv.innerHTML += ChairCat1Output;
            }
            //stoelen klasse 2 links vh midden
            for (c = 0 ; c < rij[ab][2]; c++) {
                id++;
                var ChairCat2Output = '<div class="Chair2"  onmousedown="javascript:ShowPrice(2,' + id + ')"id="' + id + '"><p style="color:#3E6DB5;"> </p></div>';
                OutputDiv.innerHTML += ChairCat2Output;
            }
            //stoelen klasse 3
            for (d = 0 ; d < rij[ab][3]; d++) {
                id++;
                var ChairCat3Output = '<div class="Chair3"  onmousedown="javascript:ShowPrice(3,' + id + ')"id="' + id + '"><p style="color:#BA200B;"> </p></div>';
            ...
            for (g = 0 ; g < rij[ab][6]; g++) {
                id++;
                var NoChairOutput = '<div class="NoChair" id="' + id + '"><p class="invi"> </p></div>';
                OutputDiv.innerHTML += NoChairOutput;
            }
